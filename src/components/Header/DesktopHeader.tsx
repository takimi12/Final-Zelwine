...
const DesktopHeader: React.FC<HeaderProps> = ({ categories }) => {
  ...
  const headerClass = isWhiteBackground || 
    (scrolled && (isMainPage || isProductsCategorySubcategory || isBiznes)) ||
    (isProductsHovered && (isMainPage || isProductsCategorySubcategory || isBiznes))
    ...
    <header className={`${styles.header} ${headerClass}`} onMouseLeave={handleHeaderMouseLeave}>
      <nav>
        <div className={styles.leftSide}>
          <Link href="/" className={`${styles.logo} `} onMouseEnter={() => handleMouseEnterLogo()}>
      ...
      {filteredCategories.map(category => (
        activeCategoryId === category.product_id && category.children.length > 0 && (
          <div className={styles.wrapperSubmenu} key={category.product_id} onMouseLeave={handleMouseLeaveSubmenu}>
            <div className={` ${styles.secondLevel}`} onMouseEnter={handleMouseLeaveFourthLevel}>
            ...
            <div className={` ${styles.thirdLevel}`} >
              {category.children.map(child => (
                activeChildCategoryId === child.product_id && child.children.length > 0 && (
                  <>
            ...
            <div className={`${styles.fourthLevel}`} onMouseLeave={handleMouseLeaveFourthLevel}>
              {activeImage ? (
                <Image width={300} height={241} src={activeImage} alt="Selected" />
              ) : (
                <Kaloryfer />
              )}
            </div>
